...
async function quuiz(req, res) {
    try {
        ...
        const quizarr = new Quize({ quizename });
        await quizarr.save();

        res.status(201).json({ message: 'Question arr created successfully' });
    } catch (err) {
        console.error(err);
...
async function updatequizname(req, res) {
    try {
        const { quizename } = req.body
        const allQuize = await Quize.findByIdAndUpdate(req.params.id, { quizename }, { new: true });
        res.status(200).json({ data: allQuize });
...
async function deletionofquestionIntoquize(req, res) {
    try {
        ...
        const exsitingques = await Quize.findById(req.params.id).populate('quizemcqs')
        const tempid = await questions.findById(questionId)


        const foundMCQs = exsitingques.quizemcqs.filter(mcq => mcq._id.toString() === tempid._id.toString());
...
module.exports = {
    getAll, quuiz, updatequizname, deletequizname, insertOrupdateQuestionsToQuiz, deletionofquestionIntoquize,getallquizequestion
}